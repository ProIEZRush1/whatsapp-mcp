...
> **Enhanced build.** A fork of [lharries/whatsapp-mcp](https://github.com/lharries/whatsapp-mcp) with:
> - **Sender name** on every incoming message (not just the number)
> - **@-mentions**: writing `@<number>` auto-tags the member (renders as their name + pings them); `list_group_members` tool + `/api/group-members` to resolve names → numbers
...
> - **Group member management**: `add_group_participants` / `remove_group_participants` tools + `POST /api/group-participants` (add/remove/promote/demote) — admin-gated, with per-person result codes (409 already-in, 408 not-on-WhatsApp, 403 privacy→invite)
